<template>
  <stellar-form-card
    ...
    <div class='row q-pb-sm'>
      <div class='col'>
        <q-input filled v-model="fields.InvoiceNumber" type="text" :label="$t('fields.InvoiceNumber')" />
      </div>
    </div>
    <div class='row q-pb-sm'>
      <div class='col'>
        <q-input filled type="number"
          v-bind="validateField($v.fields.PaymentAmount)"
          v-model.trim="$v.fields.PaymentAmount.$model"
...
export default {
  ...
  validations: {
    fields: {
      ...
      PhoneNumber: {},
      InvoiceNumber: {},
      PaymentAmount: { required, decimal, one_or_more_dollars: minValue(1) },
      Comments: {}
    }
  },
  methods: {
    afterSubmit (response) {
      this.acknowledgeSuccess(
        ...
        [
          ['dialogs.success.MakingCommitment', ['words.payment']],
